      ...
      <Accordion type="single" collapsible className="w-full">
        {skills.map((skill) => (
          <AccordionItem value={skill.name} key={skill.name} className="border-primary/20">
            ...
            <AccordionContent><SkillDetails skill={skill} /></AccordionContent>
          </AccordionItem>
        ))}
      </Accordion>
    </motion.div>
          ...
          <div ref={statsRef} className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            {stats.map((stat, index) => (
              <motion.div
                ...
                className="bg-card/60 p-10 rounded-xl border border-primary/20 shadow-md hover:shadow-primary/30 transition-shadow duration-300 hover:scale-105 "
              >
